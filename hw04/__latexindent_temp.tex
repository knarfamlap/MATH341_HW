...
\hardsubproblem{Find the posterior predictive distribution, $X_*~|~X$ where $X_*$ denotes the random variable that counts the number of successes in $n_*$ future trials. MA students --- do this yourself. Other students --- use my notes and justify each step. Remember, if $W \sim \bernoulli{p}$ then $\prob{W=1} = p$. Use that trick. Simplify.}\spc{8}
...
informative Prior. Imperical Bayes. 
...
\begin{align*}
    \thetahatmmse = \expe{\theta | X} = \frac{\alpha + X}{\alpha + \beta + n} = \frac{42.3}{42.3 + 127.7 + 4} = \frac{42.3}{174} = 0.243 
\end{align*}
...
\begin{align*}
    \rho = \frac{\alpha + \beta}{\alpha + \beta + n} = \frac{42.3 . +127.7}{42.3 + 127.7 + 4} = \frac{170}{174} = 0.977
\end{align*}
...
Although $\sum x$ is 0, since you know the batting average then you can shrink your value towards the actual average despite having not a lot of data. 
...
\begin{align*}
    X^* | X \sim \betabinomial{n}{\theta + x}{\beta+n+\alpha} = \betabinomial{20}{42.3}{147.7} 
\end{align*}

\begin{align*}
    \prob{X^*|X \geq 14} = \sum_{X^* = 14}^{20} {20 \choose X^*} \frac{B(42.3, 147.7)}{B(42.3, 127.7)} 
\end{align*}
...
\begin{align*}
    \prob{X|\theta} = {n \choose X}\theta^X(1-\theta)^{n-X} \propto \theta^X(1-\theta)^{n-X}
\end{align*}

\hardsubproblem{What is the kernel of $X, n~|~\theta \sim \binomial{n}{\theta}$? Be careful...}\spc{3}

\begin{align*}
    \prob{\theta | X} = {n \choose X} \theta^X (1-\theta)^{n-X} &= \frac{n!}{X!(n-X)!}\theta^X(1-\theta)^n(1-\theta)^{-X}
\end{align*}
...
\begin{align*}
    \prob{X | \alpha, \beta} = {n \choose x}\frac{\betanot{X + \alpha}{n - x + \beta}}{\betanot{\alpha}{\beta}} \propto \betanot{X + \alpha}{n-x+\beta} 
\end{align*}

\easysubproblem{What is the kernel of $X~|~\theta \sim \poisson{\theta} := \frac{e^{-\theta}\theta^x}{x!}$?}\spc{3}

\begin{align*}
    \prob{X|\theta} = \frac{e^{-\theta}\theta^x}{x!} \propto \frac{\theta^x}{x!}
\end{align*}

\hardsubproblem{What is the kernel of $\theta~|~X \sim \poisson{\theta}$? Be careful...}\spc{3}

\begin{align*}
    \prob{\theta|X} = \frac{\exp{-\theta}\theta^X}{X!} \propto \frac{\alpha^X}{X!} 
\end{align*}
...
\begin{align*}
    \prob{X|\alpha, \beta} = \frac{1}{B(\alpha, \beta)}X^{\alpha-1}(1-X)^{\beta-1} &\propto X^{\alpha-1}(1-X)^{\beta-1} \\ 
     &\propto X^{\alpha-1}(1-X)^\beta(1-X)^{-1}
\end{align*}
...
\begin{align*}
    \prob{X|k} = {\frac {1}{2^{k/2}\Gamma (k/2)}}\;x^{k/2-1}e^{-x/2}\; \propto X^{k/2-1}\exp{\frac{-X}{2}}
\end{align*}
...
\begin{align*}
    \prob{X|N,\theta, n} = \frac{{\theta \choose X}{n-\theta \choose n-x}}{{N \choose n}} &= \frac{\frac{\theta!}{X(\theta-X)}\frac{(N-\theta)!}{(n-x)!(N-\theta-n+X)!}}{\frac{N!}{n!(N-n)!}} \\ 
    &= \frac{\theta! n! (n-\theta)!(N-n)!}{X!(\theta-X)!(n-X)!(N-\theta-n+x)!N!} \\ 
    &\propto  \frac{1}{X!(\theta-X)!(n-X)!(N-\theta-n+X)!}
\end{align*}
